...
CREATE TABLE user_profiles (
    ...
    moon_sign TEXT,
    rising_sign TEXT,
    natal_positions JSONB DEFAULT '{}',  -- {planet: {sign, degree, house}}
    natal_aspects JSONB DEFAULT '[]',     -- [{planet1, planet2, aspect, orb}]
    house_cusps JSONB DEFAULT '{}',       -- {house_number: {sign, degree}}
...
CREATE INDEX idx_user_profiles_sun_sign ON user_profiles (sun_sign);
...
CREATE TABLE generated_content (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    content_type TEXT NOT NULL,  -- 'monthly_general', 'monthly_personal', 'moon_reflection'
    user_id TEXT,  -- NULL for general content (same for all users)
    content TEXT NOT NULL,
    -- Validity period for caching
    valid_from TIMESTAMP WITH TIME ZONE NOT NULL,
    valid_until TIMESTAMP WITH TIME ZONE NOT NULL,
    -- Additional metadata (transit data, generation params, etc.)
    metadata JSONB DEFAULT '{}',
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX idx_generated_content_user ON generated_content (user_id) WHERE user_id IS NOT NULL;
...
CREATE TABLE batch_jobs (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    job_type TEXT NOT NULL,  -- 'monthly_general', 'monthly_personal', 'moon_reflection', 'all_monthly'
    status TEXT NOT NULL DEFAULT 'pending',  -- 'pending', 'running', 'completed', 'failed'
    total_users INTEGER,
    processed_users INTEGER DEFAULT 0,
    errors JSONB DEFAULT '[]',  -- [{user_id, error_message, timestamp}]
